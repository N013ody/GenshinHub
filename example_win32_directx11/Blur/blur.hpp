...
ID3D11BlendState* pBlurBlendState;   // ÇøÓò»ìºÏ×´Ì¬
ID3D11Texture2D* pBlurMaskTexture;  // Ä£ºýÃÉ°æÎÆÀí
ID3D11ShaderResourceView* pBlurMaskSRV; // ÃÉ°æÊÓÍ¼
...
void begin(const ImDrawList* d, const ImDrawCmd* cmd) {
    ...
    ID3D11Device* device = bd->pd3dDevice;
    if (!tex)
